...
export function MonitoringProvider({
  children,
  config,
}: PropsWithChildren<MonitoringProps>) {
  useEffect(() => {
    Sentry.init({
      dsn: config.dsn,
   ...
   * @return {void}
   */
  const setMonitoringTag = useCallback(
  ...
  const measurePerformance = useCallback(
    (
      ...
      const transactionSpansObj: Record<string, Sentry.Span> = {};
      ...
      return {
        ...
        setMeasurement: (
          name: string,
          value: number,
          unit: Parameters<Transaction["setMeasurement"]>[2]
        ) => {
          transaction.setMeasurement(name, value, unit);
        },
        startSpan: (
          spanOp: string,
          spanData: { [key: string]: number | string } | null
        ) => {
          transactionSpansObj[spanOp] = transaction.startChild({
            op: spanOp,
            data: spanData ?? undefined,
          });
        },
